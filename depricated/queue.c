...
node_t *queue_insert(queue_t *queue, unsigned long long key, int val)
{
...
void queue_decrese_key(queue_t *queue, node_t *node, unsigned long long new_key)
{
    if (!node)
        return;
    if (new_key >= node->key)
